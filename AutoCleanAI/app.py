...
class App:
    ...
    def run(self):
        ...
        if uploaded_file is not None:
            ...
            # Extract numbers (if needed) and convert data type
            ...
            extract_numbers = st.radio(" ", ["Yes", "No"])

            # Select columns for extracting numbers
            text_columns = df.select_dtypes(include=['object']).columns.tolist()
            selected_columns = []

            if extract_numbers == "Yes":
                st.markdown('<div class="extract-number">Select columns to extract numbers</div>', unsafe_allow_html=True)
                selected_columns = st.multiselect(" ", text_columns)
                if st.button("Extract number and convert data type"):
                    df = CorrectType.extract_numbers_from_text(df, selected_columns)
                    df = CorrectType.correct_dtype(df, df.columns)
                    st.write(df)  
            elif st.button("Convert data type"):
                df = CorrectType.correct_dtype(df, df.columns)  
                st.write(df)
            ...
            if plot_type == "Single variable distribution":
                if st.button("Generate chart"):
                    figs = Visualisation.single_value_distribution(df)
                ...
                st.markdown('<div class="plot-instruction">Select two variables, including at least one numerical variable</div>', unsafe_allow_html=True)
                ...
                if st.button("Generate chart"):
                    if len(selected_col) != 2:
                        st.warning("⚠️ Please select two variables to explore their relationship.")
                        ...
                        # Boxplot
                        fig = Visualisation.boxplot(df, numerical_var, second_var)
                        st.plotly_chart(fig) 
            ...
            if st.button("Clean"):
                ...
                if missing_values:
                    ...
                    message.append(f" Total missing values removed: {removed} rows")

                if duplicate:
                    df, removed = tool.remove_duplicates(df)
                    message.append(f" Total duplicates removed: {removed} rows")
                
                if outliers and outliers_columns:
                    df, removed = tool.handle_outliers(df, outliers_columns)
                    message.append(f" Total outliers removed: {removed} rows")
